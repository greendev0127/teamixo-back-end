...
router.post("/start", async (req, res) => {
  try {
    ...
    const params = {
      ...
      UpdateExpression: "SET #clocked_state = :clocked_state, #track_id = :track_id, #site_id = :site_id, #last_start_date = :last_start_date, updateAt = :updateAt",
      ReturnValues: "ALL_NEW"
    };
...
router.post("/end", async (req, res) => {
  try {
    ...
    const params = {
      ...
      UpdateExpression: "SET #clocked_state = :clocked_state, updateAt = :updateAt",
      ReturnValues: "ALL_NEW"
    };
    const result = await dynamoDb.update(params).promise();
    return res.status(200).json(response);
...
router.post("/addtrack", async (req, res) => {
  try {
    ...
    const staffParams = {
      ...
      UpdateExpression: "SET #track_id = :track_id, #last_start_date = :last_start_date, #clocked_state = :clocked_state, #site_id = :site_id, updateAt = :updateAt",
      ReturnValues: "ALL_NEW"
    };
    await dynamoDb.update(staffParams).promise();
    return res.status(200).json({
...
router.post("/updatetrack", async (req, res) => {
  try {
    ...
    const dateParam = {
      ...
      UpdateExpression: "SET #name = :name, #staff_id = :staff_id, #date = :date, #start_date = :start_date, #end_date = :end_date, #total_time = :total_time, edit_state = :edit_state, updateAt = :updateAt",
      ReturnValues: "ALL_NEW"
    };
    await dynamoDb.update(dateParam).promise();
    if (data.state) {
      if (!data.subState) {
        ...
        const params = {
          ...
          UpdateExpression: "SET #last_start_date = :last_start_date, #clocked_state = :clocked_state, #track_id = :track_id, #site_id = :site_id, updateAt = :updateAt",
          ReturnValues: "ALL_NEW"
        };
        await dynamoDb.update(params).promise();
      } else {
...
router.post("/update", async (req, res) => {
  try {
    ...
    const params = {
      ...
      UpdateExpression: "SET #name_text = :name, #description_text = :description, #round = :round, #remote = :remote, updateAt = :updateAt",
      ReturnValues: "ALL_NEW"
    };
    await dynamoDb.update(params).promise();
    return res.status(200).json({
